#!/bin/bash
# ============================================================
# Open Family Cloud — パターン3: 自宅サーバー + 固定IP ヘルスチェック
# 全サービスの稼働状態を確認します
# ============================================================
set -euo pipefail
...
# 外部からのポートフォワーディング到達確認
check_port_forwarding() {
    local label=$1
    local port=$2
    local protocol=${3:-tcp}

    local global_ip="${JVB_ADVERTISE_IP}"

    if [[ -z "$global_ip" || "$global_ip" = "203.0.113.1" ]]; then
        echo -e "  ${OFC_YELLOW}△${OFC_NC} ${label}: JVB_ADVERTISE_IP が設定されていません（スキップ）"
        ((WARN++))
        return
    fi

    if [[ "$protocol" = "tcp" ]]; then
        if timeout 5 bash -c "echo >/dev/tcp/${global_ip}/${port}" 2>/dev/null; then
            echo -e "  ${OFC_GREEN}✓${OFC_NC} ${label} (${global_ip}:${port}/${protocol})"
            ((PASS++))
        else
            echo -e "  ${OFC_RED}✗${OFC_NC} ${label} (${global_ip}:${port}/${protocol} — ポートフォワーディングを確認してください)"
            ((FAIL++))
        fi
    else
        # UDP はシンプルな到達確認が困難なため警告のみ
        echo -e "  ${OFC_YELLOW}△${OFC_NC} ${label} (${global_ip}:${port}/${protocol} — UDP は自動確認不可。手動で確認してください)"
        ((WARN++))
    fi
}
...
check_disk "NAS メール" "${MAIL_STORAGE_PATH}" 80
check_disk "NAS メディア" "${JELLYFIN_MEDIA_PATH:-/mnt/nas/media}" 90
check_disk "NAS バックアップ" "${BACKUP_LOCAL_PATH:-/mnt/nas/backup}" 80
echo ""
