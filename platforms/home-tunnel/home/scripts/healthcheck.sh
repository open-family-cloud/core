#!/bin/bash
# ============================================================
# Open Family Cloud — パターン4: 自宅サーバー ヘルスチェック
# 全サービスの稼働状態 + WireGuard トンネル接続を確認します
# ============================================================
set -euo pipefail
...
echo ""
echo "===== Open Family Cloud ヘルスチェック（自宅サーバー） ====="
echo "$(date '+%Y-%m-%d %H:%M:%S')"
echo ""

# --- WireGuard トンネル ---
echo "■ WireGuard トンネル"
WG_SERVER="${WG_SERVER_IP:-10.100.0.1}"

if docker inspect --format='{{.State.Status}}' ofc-wireguard 2>/dev/null | grep -q running; then
    echo -e "  ${OFC_GREEN}✓${OFC_NC} ofc-wireguard コンテナ"
    ((PASS++))

    # VPS へのトンネル疎通確認
    if docker exec ofc-wireguard ping -c 1 -W 3 "$WG_SERVER" &>/dev/null; then
        echo -e "  ${OFC_GREEN}✓${OFC_NC} VPS への疎通 (${WG_SERVER})"
        ((PASS++))
    else
        echo -e "  ${OFC_RED}✗${OFC_NC} VPS への疎通 (${WG_SERVER}) — トンネル切断の可能性"
        ((FAIL++))
    fi

    # WireGuard ハンドシェイク確認
    LAST_HANDSHAKE=$(docker exec ofc-wireguard wg show wg0 latest-handshakes 2>/dev/null | awk '{print $2}' || echo "0")
    if [[ "$LAST_HANDSHAKE" != "0" && -n "$LAST_HANDSHAKE" ]]; then
        ...
        echo -e "  ${OFC_YELLOW}△${OFC_NC} WireGuard ハンドシェイク情報を取得できません"
        ((WARN++))
    fi
else
    echo -e "  ${OFC_RED}✗${OFC_NC} ofc-wireguard コンテナが起動していません"
    ((FAIL++))
fi
echo ""
...
check_disk "メール (NAS)" "${MAIL_STORAGE_PATH}" 80
check_disk "メディア (NAS)" "${JELLYFIN_MEDIA_PATH:-/mnt/nas/media}" 80
echo ""
